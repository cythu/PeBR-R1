...
class RayPPOTrainer:
    ...
    def _make_batch_data(self, metrics: Dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # generate a batch
            gen_batch_output = self.actor_rollout_ref_wg.generate_sequences(gen_batch)    #rollout生成结果
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("gen", timing_raw):
                    self.actor_rollout_ref_wg.prepare_rollout_engine()
                    batch = self._make_batch_data(metrics=metrics)  # <-- 真正执行推理生成 response 的函数
                    self.actor_rollout_ref_wg.release_rollout_engine()
                ...
                if "token_level_scores" not in batch.batch:
                    with timer("reward", timing_raw):
                        reward_ref = self.reward_fn.compute_reward.remote(batch)  #reward计算
    ...
    def run_rollout(self):
        ...
        """
        Rollout-only + reward-only 模式，不进行训练更新。
        """
        self.logger = Tracker(loggers=self.config.trainer.logger, config=self.config.to_dict())
        self.global_step = 0
        main_tqdm = tqdm(range(self.training_steps), desc="Running step", position=0)
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("save",timing_raw):
                    results = ray.get(self.reward_fn.save_batch_responses.remote(batch))
                    save_path = f"_rollout_outputs/rollout_results.jsonl"
